...
function Customers({ api, user, notify }) {
  ...
  async function verifyOnboardingOtp() {
    ...
    try {
      ...
      setOnboardingOtp(current=>({...current,otp:'',attempts_remaining:remaining?Number(remaining[1]):err.message.toLowerCase().includes('locked')?0:current.attempts_remaining}));
